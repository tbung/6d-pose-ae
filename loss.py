...
class Norm_Image(nn.Module):
    ...
    def __init__(self, mean_in=[0, 0, 0], std_in=[1, 1, 1]):
        ...
        self.mean = (torch.tensor(mean_in) -
                     torch.tensor([0.485, 0.456, 0.406])) / self.fac
        self.fac = torch.tensor(std_in) / self.fac

        self.mean = nn.Parameter(self.mean.view(
            1, 3, 1, 1), requires_grad=False)
...
def main():
    ...
    print("loss_0")
    loss_0 = loss_mod([zero_batch, zero_batch], [
                      ones_batch, ones_batch], [z0, z1], mode='both')
    print(loss_0)

    print("loss_1")
    loss_1 = loss_mod([zero_batch, zero_batch], [ones_batch,
                                                 ones_batch], [z0, z1], mode='no_trans')
    print(loss_1)
    print("loss_2")
    loss_2 = loss_mod([zero_batch, zero_batch], [ones_batch,
                                                 ones_batch], [z0, z1], mode='no_rot')
    print(loss_2)
